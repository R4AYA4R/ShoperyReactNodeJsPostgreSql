...
const UserFormComponent = () => {
    ...
    const [inputPassSignIn, setInputPassSignIn] = useState('');
    ...
    const login = async (email:string,password:string)=>{
        ...
        try{
            ...
            setSignInError(e.response?.data?.message + '. Fill in all fields correctly'); // помещаем в состояние ошибки формы регистрации текст ошибки,которая пришла от сервера(в данном случае еще и допольнительный текст)
                    ...
                    {tab === 'Login' &&
                        <div className="sectionUserForm__login">
                            ...
                            <div className="sectionUserForm__login-passwordBlock">
                                <input type={passwordActiveLogin ? "password" : "text"} className="sectionUserForm__login-input sectionUserForm__login-inputPassword" placeholder="Password" value={inputPassSignIn} onChange={(e)=>setInputPassSignIn(e.target.value)}/>
