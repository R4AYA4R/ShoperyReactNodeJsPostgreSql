...
class UserController {
    ...
    // функция для получения объекта админ полей(текста и тд на сайте),которые сможет админ менять на сайте
    async getAdminFields(req, res, next) {
        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const adminFields = await models.AdminFields.findOne({where:{id:1}}); // ищем объект у таблицы AdminFields в базе данных и помещаем его в переменную adminFields,у которого id равен 1,находим этот объект(он будет один со всеми нужными поля,так как мы его так создали в базе данных),в нем будут поля с значением текстов и разных полей на сайте,ищем и будем отображать потом на сайте,чтобы потом мог админ изменять их в базе данных

            return res.json(adminFields); // возвращаем на клиент объект adminFields
    ...
    // функция для изменения объекта админ полей(текста и тд на сайте)
    async changeAdminFields(req, res, next) {
        // оборачиваем в блок try catch,чтобы отлавливать ошибки
        try {

            const adminFields = req.body; // достаем(деструктуризируем) из тела запроса весь объект запроса со всеми полями,которые мы передали с фронтенда(не используем здесь деструктуризацию типа деструктурировать из req.body {adminFields} в квадратных скобках,так как просто берем все тело запроса,то есть весь объект тела запроса,а не отдельные поля)

            // если adminFields.phoneNumber.trim() не равно пустой строке,то есть поле номера телефона,которое мы взяли из тела запроса,отфильтрованное по пробелам(то есть пробелы убрались из этого поля с помощью trim()) не равное пустой строке,то есть в этом поле номера телефона есть значение,то изменяем объект адми полей,в другом случае показываем ошибку
            if(adminFields.phoneNumber.trim() !== ''){

                const updatedAdminFields = await models.AdminFields.update({...adminFields},{where:{id:1}}); // изменяем объект админ полей в базей данных,разворачиваем туда все поля у adminFields(объект,который мы взяли из тела запроса,вместо ...adminFields будут подставлены все поля,которые есть у adminFields,в данном случае так и заменяем значения старых полей на новые),у которых поле id равное 1 и помещаем этот измененный объект в переменную updatedAdminFields

                
                return res.json(updatedAdminFields); // возвращаем на клиент  измененный объект админ полей

            }else{
                return next(ApiError.BadRequest('Enter phone number correctly')); // бросаем ошибку,возвращаем с помощью return нашу функцию next(это наш errorMiddleware) и передаем туда ошибку с помощью нашего ApiError и в BadRequest(наша функция для ошибки) передаем сообщение ошибки,не используем тут throw ApiError,так как это родительский компонент ошибки(то есть если и будет ошибка,то она будет только тут) и тут есть сразу функция next в параметрах этой функции changeAdminFields,указываем return,чтобы код ниже не работал,если будет ошибка,но можно тут указать и throw ApiError
            } 
