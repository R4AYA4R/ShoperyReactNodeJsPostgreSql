...
Product.hasMany(Comment); // указываем,что объект из таблицы Product будет иметь одну связть с объектом из таблицы Comment,но поле с ссылкой на объект из таблицы Product будет создано у объекта из таблицы Comment(то есть в данном случае у объекта из таблицы Comment будет поле productId,где будет указано значение id объекта из таблицы Product,с которым этот объект из таблицы Comment связан), это касается связываний типа hasOne,hasMany
